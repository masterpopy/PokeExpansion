...
void BuildEggMoveset(struct pokemon* egg, struct pokemon* father, struct pokemon* mother)
{
	int numSharedParentMoves;
	int numLevelUpMoves;
	int numEggMoves;
	int i, j;
	u16 sHatchedEggLevelUpMoves[EGG_LVL_UP_MOVES_ARRAY_COUNT] = { 0 };
	u16 sHatchedEggFatherMoves[4] = { 0 };
	u16 sHatchedEggFinalMoves[4] = { 0 };
	u16 sHatchedEggEggMoves[EGG_MOVES_ARRAY_COUNT] = { 0 };
	u16 sHatchedEggMotherMoves[4] = { 0 };
	...
	numLevelUpMoves = GetLevelUpMovesBySpecies(get_attributes(egg, ATTR_SPECIES, 0), sHatchedEggLevelUpMoves);
	for (i = 0; i < MAX_MON_MOVES; i++)
	{
		sHatchedEggFatherMoves[i] = get_attributes(father, ATTR_ATTACK_1 + i, 0);
		sHatchedEggMotherMoves[i] = get_attributes(mother, ATTR_ATTACK_1 + i, 0);
	}
	...
	for (i = 0; i < MAX_MON_MOVES; i++)
	{
		if (sHatchedEggFatherMoves[i] != MOVE_NONE)
		{
			...
			{
				if (sHatchedEggFatherMoves[i] == get_TM_move(ITEM_TM01_FOCUS_PUNCH + j) &&
					check_TM_compatibility2(egg, j))
				{
					if (GiveMoveToMon(egg, sHatchedEggFatherMoves[i]) == MON_HAS_MAX_MOVES)
						DeleteFirstMoveAndGiveMoveToMon(egg, sHatchedEggFatherMoves[i]);
				}
